...
func main() {
	Makefile(
		...
		// cli tests: native go-make Task. Requires SYFT_BINARY_LOCATION to point at
		// a snapshot binary, so we depend on snapshot and assert the bin exists.
		Task{
			Name:         "cli",
...
// snapshot/<os>-build_<os>_<arch>/syft, where arch maps amd64->amd64_v1 and
// arm64->arm64_v8.0 to match goreleaser's per-target output directory naming.
func snapshotBinPath() string {
	...
	return filepath.Join("snapshot", osName+"-build_"+osName+"_"+arch, "syft")
}
